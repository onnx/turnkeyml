...
Name of one or more script (.py), ONNX (.onnx), or cached build (state.yaml) files to be evaluated. You may also specify a (.txt) that lists file paths separated by line breaks.
...
- The state file, `state.yaml`, which contains all of the build state required to load that build into a new sequence using the `load-build` tool.
